...
class _LibraryScreenState extends ConsumerState<LibraryScreen> {
  ...
  /// Makes the library match the disk: re-parses metadata for every
  /// already-indexed track (a normal scan skips files it already knows, so
  /// stale tags would otherwise never be fixed) and removes ones whose file
  /// is gone — then looks for NEW songs in every folder the user has added
  /// before. The refresh runs first on purpose: it only covers tracks already
  /// in the library, so newly found songs aren't read a second time.
  Future<void> _rescanLibrary() async {
    ...
    try {
      ...
      final folders = ref.read(appSettingsProvider).importedFolders;
      ScanResult? found;
      if (!result.wasCancelled && folders.isNotEmpty) {
        if (mounted) {
          setState(() {
            _scanProgress = 0;
            _scanTotal = null;
        ...
        found = await scanner.scanFolders(
          folders,
          cancellationToken: token,
      ...
      if (mounted) {
        final newCount = found?.addedCount ?? 0;
        final base = folders.isEmpty
            ? 'Refreshed ${result.processed} track${result.processed == 1 ? '' : 's'}. '
                  'No folders to look in yet — use Add Folder once so new songs '
                  'can be found later'
            : newCount > 0
            ? 'Found $newCount new song${newCount == 1 ? '' : 's'}'
            : 'No new songs found';
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              _withFailureNote(
                withRemoved,
                found?.failedPaths.length ?? 0,
                wasCancelled:
                    result.wasCancelled || (found?.wasCancelled ?? false),
              ),
            ),
          ),
        );
  ...
  Future<void> _pickAndScanFolder() async {
    ...
    try {
      ...
      // Remembered so "Rescan library" can look here for new songs later
      // (see LibraryScanner.scanFolders).
    ...
    return Scaffold(
      appBar: _selectionMode
          ...
          : AppBar(
              ...
              actions: [
                IconButton(
                  tooltip: 'Rescan library (find new songs, refresh tags)',
                  icon: const ThemedIcon(ThemedIconSlot.refresh),
                  onPressed: _scanning ? null : _rescanLibrary,
                ),
                IconButton(
                  tooltip: 'Add Files',
